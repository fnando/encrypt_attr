...
class EncryptAttrTest < Minitest::Test
  ...
  %w[
    ...
    test "encrypts attribute using alias method - #{encryption_method}" do
      ...
      new_instance = klass.new(encrypted_api_key: instance.encrypted_api_key)
      assert_equal "API_KEY", new_instance.api_key
    end
  end
  ...
  test "encrypts one attribute using default secret token" do
    ...
    assert_equal "API_KEY", instance.api_key
    refute_equal "API_KEY", instance.encrypted_api_key
  ...
  test "encrypts multiple attributes using default secret token" do
    ...
    new_instance = klass.new(encrypted_api_key: instance.encrypted_api_key, encrypted_api_client_id: instance.encrypted_api_client_id)
    assert_equal "API_KEY", new_instance.api_key
    assert_equal "API_CLIENT_ID", new_instance.api_client_id
  end
  ...
  test "updates encrypted value" do
    ...
    new_instance = klass.new(encrypted_api_key: instance.encrypted_api_key)
    assert_equal "NEW_API_KEY", new_instance.api_key
  end

  [nil, ""].each do |value|
    test "skips value (#{value.inspect})" do
      klass = create_class do
        attr_accessor :encrypted_api_key
        encrypt_attr :api_key
      end
      ...
      instance = klass.new(api_key: "API_KEY")
      instance.api_key = value

      assert_same value, instance.api_key
